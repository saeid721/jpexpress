(() => {
  ...
  const render = () => {
    ...
    const sorted = [...cards].sort((a, b) => sort.value === 'read'
      ? Number(a.dataset.read) - Number(b.dataset.read)
      : Number(b.querySelector('time').dateTime.replaceAll('-', '')) - Number(a.querySelector('time').dateTime.replaceAll('-', '')));
    ...
    sorted.forEach((card, index) => {
      ...
      const passes = categoryMatch && searchMatch;
      const show = passes && index < visibleLimit;
  ...
  newsletter.addEventListener('submit', event => {
    event.preventDefault();
    if (!newsletter.checkValidity()) { newsletter.querySelector(':invalid').focus(); return; }
    document.getElementById('newsletterSuccess').classList.add('visible');
    newsletter.reset();
  });

  /* Featured-article "copy link" share button */
  const copyBtn = document.getElementById('copyLinkBtn');
  if (copyBtn) {
    copyBtn.addEventListener('click', e => {
      e.preventDefault();
      const url = window.location.href.split('#')[0] + '#article-library';
